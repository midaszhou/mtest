...
//+++++-----ISO set dev_ctx->vr_params.end_of_frame_type =  EOF_PENDING_BIT
	dev_ctx->vr_params.end_of_frame_type = EOF_ZERO_LENGTH;
//	dev_ctx->vr_params.end_of_frame_type = EOF_PENDING_BIT;
//------------------------------end of ISO
	...
	if (dev_ctx->registry.CompressionEnable ||
		...
		if (dev_ctx->registry.Usb2PixelFormatTransformCompressionEnable) {
			...
			//
//+++++COMP change dev_ctx->vr_params.compression_mask_index = COMPRESSION_MASK_13_BIT_INDEX
			dev_ctx->vr_params.compression_mask = COMPRESSION_MASK_15_BIT_VALUE;
			dev_ctx->vr_params.compression_mask_index = COMPRESSION_MASK_15_BIT_INDEX; //13_BIT_INDEX;
...
//+++++ISO change case RGB_16: table_num = VGA_BIG_TABLE_16BIT_R0 to R1
//		table_num = VGA_BIG_TABLE_16BIT_R0;
		table_num = VGA_BIG_TABLE_16BIT_R1;
//--------------------end of ISO
		break;
...
//+++++-----ISO  suspend  fl2000_monitor_set_resolution(dev_ctx, pll_changed), and use defualt setting...
...
//+++++-----ISO set cur_altsetting->desc.bAlternateSetting = dev_ctx->usb_ifc_streaming->altsetting[1]
//	dev_ctx->usb_ifc_streaming->cur_altsetting = &dev_ctx->usb_ifc_streaming->altsetting[0];   //->desc.bInterfaceNumber=1;
//--------------------------- end of ISO

//+++++ISO comment out here as conflict to fl2000_dev_init(struct dev_ctx * dev_ctx)
	// Select Interface
/*
...
//+++++ISO  BOTH usb_sndbulkpipe() AND  usb_sndisocpipe()
	dev_ctx->usb_pipe_bulk_out = usb_sndbulkpipe(dev_ctx->usb_dev, 1);
	printk("fl2000_dongle_set_params: usb_pipe_bulk_out= 0x%x  ----------\n",dev_ctx->usb_pipe_bulk_out);
	dev_ctx->usb_pipe_iso_out = usb_sndisocpipe(dev_ctx->usb_dev, 2);
	printk("fl2000_dongle_set_params: usb_pipe_iso_out= 0x%x  ----------\n",dev_ctx->usb_pipe_iso_out);
//----------end of ISO
...
//+++++ISO chang vr_params.trasfer_pipe = VR_TRANSFER_PIPE_BULK to VR_TRANSFER_PIPE_ISOCH;
//	vr_params.trasfer_pipe = VR_TRANSFER_PIPE_BULK;
	vr_params.trasfer_pipe = VR_TRANSFER_PIPE_ISOCH;
//----------------end of ISO

//+++++COMP set  vr_params.use_compression = display_mode->use_compression;
