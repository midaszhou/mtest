...
//+++++ISO set cur_altsetting = &dev_ctx->usb_ifc_streaming->altsetting[1]
//	cur_altsetting = dev_ctx->usb_ifc_streaming->cur_altsetting;
	if(dev_ctx->usb_ifc_streaming->cur_altsetting ==  &dev_ctx->usb_ifc_streaming->altsetting[0])
		cur_altsetting = &dev_ctx->usb_ifc_streaming->altsetting[1];
	else
		cur_altsetting = &dev_ctx->usb_ifc_streaming->altsetting[0];
	//--renew dev_ctx->usb_ifc_streaming->cur_altsetting
	dev_ctx->usb_ifc_streaming->cur_altsetting=cur_altsetting;
//------------------end of ISO
...
//+++++-----ISO change cur_altsetting->desc.bAlternateSetting to 1
...
//-------------end of ISO
        ...
        dbg_msg(TRACE_LEVEL_INFO, DBG_PNP,
                ...
                dev_ctx->usb_ifc_streaming->cur_altsetting->desc.bInterfaceNumber,
                dev_ctx->usb_ifc_streaming->cur_altsetting->desc.bAlternateSetting);
	...
	if (0 != ret_val) {
		...
		printk("------usb_set_interface() change cur_altsetting->desc.bAlternateSetting to 1 successfully!\n");
