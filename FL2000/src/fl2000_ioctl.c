...
*/
	...
	if (surface->render_buffer == surface->shadow_buffer) {
		 ...
		 * SURFACE_TYPE_VIRTUAL_FRAGMENTED_VOLATILE (0) , and is not
		 * yet locked down.
		 */
		if (!surface->pre_locked &&
			...
			//+++++COMP  if use_compression, then self_pixel_swap(surface->system_buffer, surface->buffer_length),later in fl2000_compression_gravity()
			//we'll compress surface->system_buffer to surface->render_buffer(=shadow_buffer)
			if(dev_ctx->vr_params.use_compression){
				self_pixel_swap(surface->system_buffer, surface->buffer_length);
			}
			//if NOT use_compression,then swap
			else{
...
//-------- since it's VOLATILE , so unlock surface ????? -------------
unlock_surface:
		...
		//================  else if SURFACE_TYPE_VIRTUAL_FRAGMENTED_PERSISTENT (1) =========
		else {
		...
		//+++++COMP  if use_compression, then pixel_swap() system_buffer to shadow_buffer,later in fl2000_compression_gravity() \
		// we'll compress surface->shadow_buffer to surface->render_buffer(=system_buffer@@@@@)
		if(dev_ctx->vr_params.use_compression){
				pixel_swap(surface->shadow_buffer,
					surface->system_buffer,
					surface->buffer_length);
		}
		//if NOT use_compression,then self swap
		else{
