...
// !!!!  surface_info.pitch and color_format will be used to create surface and pin down pages
	...
	surface_info.color_format	= COLOR_FORMAT_RGB_16_565;//565;//COLOR_FORMAT_RGB_24; 
	...
	display_mode.input_color_format = COLOR_FORMAT_RGB_16_565;//565;//24;
	...
	display_mode.output_color_format =COLOR_FORMAT_RGB_16_565;//565;//24;//16_555;//16_565;//COLOR_FORMAT_RGB_24;
//+++++COMP add display_mode.use_compression = 1;
	display_mode.use_compression = 0;
...
usage:
	...
	if (argc >= 4) {
        ...
        {
		//test_display_all(fd);
//NOTE: in kernel driver, we set surface->render_buffer = surface->system_buffer in case of VIRTUAL_CONTIGUOUS and PHYSICAL_GONTIGUOUS mem type
		printf("---test_display()-----\n");
		//test_display(fd,400,300,0);
		test_display(fd,640,480,5);
//		test_display(fd,640,350,0);
//		test_display(fd,720,400,0);
//		test_display(fd,800,600,5);
